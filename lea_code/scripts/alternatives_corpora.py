...
full_alt = re.compile(r'(\( ?\d+ ?\))? *alternative [abc] ?[.,]*')
...
def get_alternatives_corpus(fname, outdir=""):
    ...
    for paragraph in content.split("\n\n"):
        ...
        for alt in [['a', 'b', 'c'], ['b', 'a', 'c'], ['c', 'b', 'a']]:
            if dist[alt[0]]>0 and (dist[alt[1]]==dist[alt[2]]==0 and len(dist['two'])==0):
                pid="-1"
                ...
                alt_sents = [' '.join([alt[0], 'SENT', '--', sent]) for sent in text if 'alternative {} '.format(alt[0]) in sent and not ' {} '.format(alt[1]) in sent and not ' {} '.format(alt[2]) in sent and not full_alt.match(sent)]
                if len(alt_sents) > 0 :
...
def main():
    indir = "/home/lea/academic/melbourne/research/fed_causal/data/bluebook_raw_text/paragraphs_dist"
    outdir = "/home/lea/academic/melbourne/research/fed_causal/data/bluebook_raw_text/alternative_corpora2"
