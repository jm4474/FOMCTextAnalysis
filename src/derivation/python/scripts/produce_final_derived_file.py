...
def merge_nyt(cur_df):
    ...
    df_value_counts.columns = ['end_date', 'nyt_article_counts']
    cur_df['end_date'] = pd.to_datetime(cur_df['end_date'])
    df_value_counts['end_date'] = pd.to_datetime(df_value_counts['end_date'])
    merge_nyt = cur_df.merge(df_value_counts,on="end_date",how="left")
    merge_nyt['nyt_article_counts'].fillna(0,inplace=True)
    return merge_nyt

def merge_bluebook(cur_df):
    print("merging bluebook")
    blue_df = pd.read_excel("../data/bluebook_manual_data_online.xlsx")
