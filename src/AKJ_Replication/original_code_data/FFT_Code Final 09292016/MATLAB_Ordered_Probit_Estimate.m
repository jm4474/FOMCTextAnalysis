...
%
%
%  Modified             :   Guido Kuersteiner
%  Note                 :   Added predicted probabilities
%
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%           *Usage:*
...
%----------------------------------------------------------------------------------------------------------------         
    ...
    randn('seed',0);
    while NoConv
        % run optimizer until random initial par values lead to maximum
    ...
    %----------------------------------------------------------------------------------------------------------------         
        ...
        if ones(length(RESULTS.Beta),1)'*isnan(RESULTS.Beta)==0
            NoConv = false;
        end
    end
...
%----------------------------------------------------------------------------------------------------------------        
    ...
    CP   = RESULTS.Cut_Points;
    nCP  = size(CP,1);
    n    = size(xb,1);
    phat = zeros(n,nCP+1);
    for i = 1:nCP
        if i>1
           phat(:,i) = normcdf(CP(i,1)*ones(n,1)-xb)-normcdf(CP(i-1,1)*ones(n,1)-xb);
        else
           phat(:,i) = normcdf(CP(i,1)*ones(n,1)-xb);
        end
    end
    phat(:,nCP+1)  = ones(n,1)-normcdf(CP(nCP,1)*ones(n,1)-xb);
    RESULTS.phat = phat;
...
%     t_statistic = [RESULTS.Beta;RESULTS.Cut_Points]./se;
    ...
    % User would like to see some output.
        ...
        % -------------------------------------------------------------------------------------------
        % Use the user-provided variables names, if exists
        % -------------------------------------------------------------------------------------------
        if isfield(INPUTS,'Names')
            if length(INPUTS.Names) == length(RESULTS.Beta)
                Row_Names_Provided = 1;
            else
                Row_Names_Provided = 0;
            end
        else
            Row_Names_Provided = 0;
        end        
        ...
        for i=1:length(RESULTS.Beta)
            if Row_Names_Provided == 1
                Row_Heading{Row_Name_Count} = INPUTS.Names{Row_Name_Count};
            else
        ...
        fprintf('\n');
        fprintf('%20s','variable','coeff','s.e.','t-value');
        fprintf('\n');
        for i=1:Row_Name_Count-1    
            fprintf('%20s',Row_Heading{i}); %print ith variable name    
            fprintf('%20.4f',[Estimated_Coefficient(i,:) se(i,:) z_statistic(i,:)     ]);%print results for variable i        
            fprintf('\n');
        end
        fprintf('-----------------------------------------------------------------------------------------------')
        fprintf('\n');
