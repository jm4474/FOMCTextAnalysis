function [Model,Data] = LoadDataMonthly(Model)
...
[s,y,~,ind,dyMonth]     = MergeOutcomeVarFiles(respvfn,respvfn1,Start,End);
...
% This code section allows to exclude observations of the outcome variable
% from entering the impulse response averages. This feature is currently not
% used.
%for i = 1:size(yM,2);
%    if ~isempty(Model.ExcludeOutcomeList(i).ExcludeDate)
%        [~,id,~] = intersect(dyMonth,datenum(Model.ExcludeOutcomeList(i).ExcludeDate));
%        yM(id,i) = zeros(length(id),1); %set values to be excluded = 0, this is equivalent to not couting them in the sum 
%    end
%end
...
TargetRate   = data(:,strcmp(colheaders,'Ctarget'));
...
for i = 1:length(data);
    if ~(month(i,1)==1)
        monDum(i,month(i,1)) = 1;
    end
end

data       =         [data,monDum(:,2:12)];
                
                

%Define FOMC-change-only dep variable
yChange    = data(:,strcmp(colheaders,'RRdtarget')).*data(:,strcmp(colheaders,'FOMC meeting'));
data       = [data,yChange];
colheaders = [colheaders,'RRdtargetFOMC'];

%Define FOMC-change-only lag dep variable
yChange    = data(:,strcmp(colheaders,'LastChange')).*data(:,strcmp(colheaders,'FOMC meeting'));
data       = [data,yChange];
colheaders = [colheaders,'LastCFOMC'];
