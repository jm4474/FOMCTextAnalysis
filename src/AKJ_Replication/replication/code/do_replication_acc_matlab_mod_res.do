...
foreach var of varlist d1_ln_pceh d1_ln_ip d1_ffed d1_unrate{
	...
	* Residuals of weights
foreach element in _m050 _m025 _0 _025 _050{
	quietly reg delta`element' $covariates ///
	if d_sample1==1 & yhat`element' != ., noconstant 			
	predict delta`element'_res ///
	if d_sample1==1 & yhat`element' != ., residuals 				
}
...
foreach policy in `policies'{
	foreach var of varlist  d1_ln_pceh d1_ln_ip d1_unrate{
		foreach horizon of numlist 1/24{
			...
			capture replace se_`var'_`policy'=_se[_cons]  ///
			if horizon==`horizon'
...
graph export ../output/replication_real_res_weights.pdf,replace
