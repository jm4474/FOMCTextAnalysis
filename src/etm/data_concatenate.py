...
def data_clean(docs,th="inf",phrase_itera=2):
    ...
    additional_stopword = ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september',
                                   ...
                                   "recessed","laugther","reception","dinner","martin","terrace",
                                   "memo","memos","ll","don","panel","appendix","left","shown","chairman",
                                   "president","chair","brian","moskow","guynn","mr","madam","blue","red","roll","california",
                                   'pianalto', 'poole', 'warsh',  'bernanke', 'hoenig', 'duke', 'bies',"laughter","handout","line",'williams']
...
def data_preprocesslist(init_docs,DATASET,phrase_itera,max_df,min_df,docdates=[],docindex=[]):
...
def data_preprocess(docs,DATASET,phrase_itera,max_df,min_df,th=10,docdates=[],docindex=[]):
    # Tokenize the documents
    init_docs = [re.findall(r'''[\w']+|[.,!?;-~{}`´_<=>:/@*()&'$%#"]''', doc) for doc in docs]
    init_docs = [[w.lower() for w in init_docs[doc] if not contains_punctuation(w)] for doc in range(len(init_docs))] # removes punct and makes lower case.
    init_docs = [[w for w in init_docs[doc] if not contains_numeric(w)] for doc in range(len(init_docs))]  # removes numeric
...
def word_count(df,name):
    ...
    df_year = df.groupby(['year'])["content"].agg(lambda x: ' '.join(x)).reset_index()["content"].to_list()
    df_year_date = df.groupby(['year'])["content"].agg(lambda x: ' '.join(x)).reset_index()['year'].to_list()
    df_year = data_clean(df_year)

    # produce doc stats
    token_counts_yr = [len(doc) for doc in df_year]
    fig, ax = plt.subplots()
    ax.plot(df_year_date,token_counts_yr)
    plt.savefig(f"output/{name}_yearly.png")

    df_date = df.groupby(['date'])["content"].agg(lambda x: ' '.join(x)).reset_index()["content"].to_list()
    df_date_date = df.groupby(['date'])["content"].agg(lambda x: ' '.join(x)).reset_index()['date'].to_list()
    df_date = data_clean(df_date)

    # produce doc stats
    token_counts = [len(doc) for doc in df_date]
    del ax
    del fig
    fig, ax = plt.subplots()
...
def build_embdata(max_df,min_df,phrase_itera,threshold,DATASET):
    ...
    data_preprocess(docs,DATASET,phrase_itera,max_df,min_df,threshold,transcripts_dates)
...
def sampling(df , max_words = 80000):
    ...
    meeting_text = data_clean(df.groupby(["date"])["content"].agg(lambda x: ' '.join(x)).reset_index()["content"].to_list())
    meeting_years = df[["date","year"]].drop_duplicates(subset=["date"]).reset_index()['year'].to_list()
    meeting_dates = df[["date","year"]].drop_duplicates(subset=["date"]).reset_index()['date'].to_list()
    ...
    return meeting_dates,meeting_text
...
# =============================================================================
    ...
    part1 = pd.DataFrame(zip(speakers_full.loc[speakers_full["Section"]==1,"start_date"].to_list(),
                                     speakers_full.loc[speakers_full["Section"]==1,"content"].to_list()),
                                 columns=["date","content"])
    
    meeting_dates,meeting_text = sampling(part1)
    part1_spld = pd.DataFrame(zip(meeting_dates,meeting_text),columns=["date","content"])
    part1_spld["Section"] = 1 

    part2 = pd.DataFrame(zip(speakers_full.loc[speakers_full["Section"]==2,"start_date"].to_list(),
                                     speakers_full.loc[speakers_full["Section"]==2,"content"].to_list()),
                                 columns=["date","content"])
    meeting_dates,meeting_text = sampling(part2,max_words=30000)
    part2_spld = pd.DataFrame(zip(meeting_dates,meeting_text),columns=["date","content"])
    part2_spld["Section"] = 2   
    
    data_sampled = pd.concat([part1_spld,part2_spld],axis=0)
    data_sampled.to_pickle(f"raw_data/{DATASET}sampled.pkl")
    
    if not os.path.exists(f"{OUTPATH}/{DATASET}sampled"):
        os.makedirs(f"{OUTPATH}/{DATASET}sampled")
        
    word_list = data_sampled["content"].to_list()
    data_preprocesslist(word_list ,f"{DATASET}sampled",phrase_itera,max_df,min_df,threshold,docindex = list(data_sampled.index))
